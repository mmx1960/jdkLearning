...
哪里有反抗，哪里就有起义，对ThreadLocal不爽，我们就来改造他。先说下我们要达到什么目的

- 改造内部存储，改成直接索引，减少hash以及遍历需要的时间
- 支持ThreadLocal原有的读写接口
- 自定义Thread类，使其支持自定义ThreadLocal
- 支持一般Thread中使用自定义ThreadLocal(未完成)

以上是我们想去实现的，具体的可以直接看源码，我就说下我的内部存储的实现。

在这里，我并没有使用hash索引，而是制定了一个id自增长的规则，因为每个Thread里都对应也只对应一个Map，那我就维护一个index序列，每次set新的实例的时候，序列的指针指向下一个，这样，我只要在resize的时候重新分配下序列，就可以了。
这样大大的减少了查找的时间，时间复杂度为O(1)，可能会在resize的时候花费时间久一点，但是，毕竟resize也不是个经常触发的操作，所以一定程度增强性能。

其中有个注意点，Entry类还是和原来一样，一个WeakReference的实现类，这意味着他一样有可能会发生被回收的问题。我简单的制定了一个决策，就是在扩容的时候，顺序查找被回收的节点，然后把节点的位置加入到index序列里，这样我新发生set操作时，空间资源也能得到有效的利用。

话不多说，还是看代码吧，后面计划加上性能分析这一块。